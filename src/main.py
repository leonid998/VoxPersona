...
async def migrate_old_indices_if_needed():
    ...
    if old_model_marker.exists() or not new_model_marker.exists():
        ...
        # Удаляем все .faiss и .pkl файлы (старые индексы)
        for file_pattern in ["*.faiss", "*.pkl"]:
            for old_file in indices_dir.glob(file_pattern):
                try:
                    old_file.unlink()
                    logging.info(f"   Удален старый индекс: {old_file.name}")
                except Exception as e:
                    logging.error(f"   Не удалось удалить {old_file.name}: {e}")
