...
# ВАЖНО: Имена отчетов ДОЛЖНЫ точно соответствовать выводу load_report_descriptions()
# из relevance_evaluator.py (имена файлов после удаления префиксов).
# Синхронизировано с REPORT_TO_INDEX_MAPPING в relevance_evaluator.py
INDEX_MAPPING: Dict[str, List[str]] = {
    ...
    "Itogovye_otchety": [
        "Главная",
        "Заполняемость_и_бронирование",
        "Итоговый_отчет",
        "Отдых_и_восстановление",
        "Ощущения_от_отеля",
        "Рекомендации_по_улучшению"
    ],
    # Otchety_po_dizaynu (5 отчетов) - Содержание_Дизайн отчеты
    "Otchety_po_dizaynu": [
        "Дизайн_и_архитектура",
        "Недостатки_дизайна",
        "Ожидания_и_реальность",
        "Противоречия_концепции_и_дизайна",
        "Сильные_стороны_дизайна"
    ],
    # Otchety_po_obsledovaniyu (5 отчетов) - Содержание_Обследование отчеты
    "Otchety_po_obsledovaniyu": [
        "Востребованность_гостиничного_хозяйства",
        "Качество_инфраструктуры",
        "Клиентский_опыт",
        "Маршруты_и_безопасность",
        "Обустройство_гостиничного_хозяйства"
    ]
}
...
# Шаг 3.1: Обновлены названия индексов для лучшей читаемости в UI - решает проблему непонятных названий
INDEX_DISPLAY_NAMES: Dict[str, str] = {
    "Dizayn": "Аудит дизайна (Казань)",
    "Intervyu": "Интервью (Казань)",
    "Iskhodniki_dizayn": "Исходники дизайн (РФ)",
    "Iskhodniki_obsledovanie": "Исходники обследование (РФ)",
    "Itogovye_otchety": "Итоговые отчеты (РФ)",
    "Otchety_po_dizaynu": "Отчеты по дизайну (РФ)",
    "Otchety_po_obsledovaniyu": "Отчеты по обследованию (РФ)"
}
...
def select_most_relevant_index(
    ...
    Examples:
        >>> report_rel = {
        ...     "Дизайн_и_архитектура": 0.95,
        ...     "Сильные_стороны_дизайна": 0.90,
        ...     "Обследование": 0.30
        ... }
...
def get_top_relevant_indices(
    ...
    Example:
        >>> relevance = {"Дизайн_и_архитектура": 95.0, "Обследование": 30.0}
