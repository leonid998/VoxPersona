...
Модуль содержит 41 функцию для управления:
- Пользователями (14 handlers)
- Приглашениями (7 handlers)
- Безопасностью (5 handlers: security menu, audit log, 3x cleanup settings)
...
async def handle_change_user_settings(chat_id: int, user_id: str, app: Client):
    """
    ...
    Args:
        chat_id: Telegram chat_id администратора
        user_id: user_id пользователя для изменения настроек
        app: Pyrogram Client
    """
    try:
        auth = get_auth_manager()
        ...
        status_blocked = "🚫 Заблокирован" if user.is_blocked else "✅ Разблокирован"
        ...
        await track_and_send(
            ...
            app=app,
            text=text,
            reply_markup=access_user_settings_markup(user_id),
            message_type="menu"
        )

        logger.info(f"User settings menu shown: chat_id={chat_id}, user_id={user_id}")

    except Exception as e:
        logger.error(f"Error in handle_change_user_settings: {e}")
        logger.error(traceback.format_exc())
        await track_and_send(
            chat_id=chat_id,
            app=app,
            text="❌ Произошла ошибка при загрузке меню настроек.",
            message_type="menu"
        )
...
async def handle_delete_user(chat_id: int, user_id: str, app: Client):
    ...
    Триггер: callback_data "access_delete_user_confirm||{user_id}"
    Следующий шаг: "access_confirm_delete||{user_id}" → handle_confirm_delete()
...
async def handle_set_cleanup_hours(chat_id: int, app: Client):
    """
    ...
    try:
        text = (
            ...
            "• Настройка TTL для меню (1-48 часов)\n"
            "• Настройка TTL для уведомлений\n"
            "• Настройка TTL для статусных сообщений\n"
            "• Индивидуальные правила для разных типов\n\n"
            "📋 **Текущая реализация:**\n"
            "• Меню очищаются при открытии нового\n"
            "• Уведомления удаляются через 15 секунд\n"
            "• Статусные сообщения через 10 секунд\n\n"
            "Расширенные настройки времени будут доступны в следующих версиях."
        )

        # TODO: Реализовать функционал установки времени автоочистки
        # - Добавить CleanupSettings в auth_models.py
        # - Создать UI для выбора времени (1-48 часов)
        # - Интегрировать с MessageTracker
        # - Добавить сохранение настроек в auth_storage

        await track_and_send(
            chat_id=chat_id,
            app=app,
            text=text,
            reply_markup=access_back_markup("access_cleanup_settings"),
            message_type="menu"
        )

        logger.info(f"Set cleanup hours screen shown to chat_id={chat_id}")

    except Exception as e:
        logger.error(f"Error in handle_set_cleanup_hours: {e}")
        await track_and_send(
            chat_id=chat_id,
            app=app,
            text="❌ Произошла ошибка при загрузке настроек времени.",
            message_type="menu"
        )


async def handle_cleanup_per_user(chat_id: int, app: Client):
    """
    ...
    try:
        text = (
            ...
            "⏳ **Функция в разработке**\n\n"
            "Планируемые возможности:\n"
            "• Индивидуальные правила очистки для каждого пользователя\n"
            "• Настройка времени жизни сообщений по ролям (admin, user, guest)\n"
            "• Отключение автоочистки для определенных пользователей\n"
            "• Настройка поведения очистки при смене контекста\n\n"
            "📋 **Текущая реализация:**\n"
            "• Единые правила для всех пользователей\n"
            "• Очистка по типам сообщений (меню, уведомления, статусы)\n"
            "• Автоматическая очистка при смене контекста\n\n"
            "🔧 **Будущие возможности:**\n"
            "• Per-user TTL настройки\n"
            "• Роль-основанные правила очистки\n"
            "• Исключения для конкретных пользователей"
        )

        # TODO: Реализовать функционал per-user настроек
        # - Расширить User модель в auth_models.py с cleanup_settings
        # - Создать UI для настройки правил для каждого пользователя
        # - Интегрировать с MessageTracker для учета user_id
        # - Добавить фильтры по ролям (admin, user, guest)
        ...
        logger.info(f"Cleanup per-user screen shown to chat_id={chat_id}")

    except Exception as e:
        logger.error(f"Error in handle_cleanup_per_user: {e}")
        await track_and_send(
            chat_id=chat_id,
            app=app,
            text="❌ Произошла ошибка при загрузке настроек пользователей.",
            message_type="menu"
        )


async def handle_view_cleanup_schedule(chat_id: int, app: Client):
    """
    ...
    try:
        text = (
            ...
            "⏳ **Функция в разработке**\n\n"
            "Планируемые возможности:\n"
            "• Просмотр текущих правил очистки\n"
            "• Статистика очищенных сообщений за период\n"
            "• Список сообщений, запланированных к удалению\n"
            "• История операций автоочистки\n"
            "• Ближайшие запланированные очистки\n\n"
            "📋 **Текущая реализация:**\n"
            "• MessageTracker отслеживает все отправленные сообщения\n"
            "• Автоочистка происходит при смене контекста\n"
            "• Логирование операций очистки в логи\n\n"
            "🔍 **Будущие возможности:**\n"
            "• Dashboard с аналитикой очистки\n"
            "• Графики использования автоочистки\n"
            "• Фильтры по пользователям/типам\n"
            "• Экспорт статистики"
        )

        # TODO: Реализовать функционал просмотра расписания
        # - Добавить CleanupSchedule модель для хранения правил
        # - Создать UI для отображения активных правил
        # - Интегрировать с audit log для статистики
        # - Добавить аналитику (количество очищенных сообщений)
        ...
        logger.info(f"Cleanup schedule screen shown to chat_id={chat_id}")

    except Exception as e:
        logger.error(f"Error in handle_view_cleanup_schedule: {e}")
        await track_and_send(
            chat_id=chat_id,
            app=app,
            text="❌ Произошла ошибка при загрузке расписания.",
            message_type="menu"
        )
