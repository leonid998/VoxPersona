...
async def handle_switch_chat_confirm(
    ...
    try:
        ...
        # ИСПРАВЛЕНО (2025-11-23): Добавлена информация о режиме и подсказка
        # ЗАЧЕМ: err_task.txt требует показать что выбран Быстрый поиск и подсказку "Введите вопрос"
        # ПОЧЕМУ: При входе в чат deep_search=False устанавливается выше, нужно об этом сообщить
        text = (
            f"✅ Переключено на чат: **{conversation.metadata.title}**\n\n"
            f"📊 Режим: **Быстрый поиск** (по умолчанию)\n\n"
            f"✏️ **Введите свой вопрос** или выберите другой режим:"
        )
