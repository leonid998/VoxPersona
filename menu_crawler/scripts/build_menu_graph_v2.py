#!/usr/bin/env python3
"""
Построитель menu_graph.json для VoxPersona
Полное ручное определение всех узлов и связей из COMPLETE_MENU_TREE.md
"""
...
class MenuGraphBuilder:
    ...
    def validate(self) -> tuple[bool, list]:
        ...
        # FSM и динамические узлы могут быть изолированными (это нормально)
        isolated_non_special = {
            node for node in isolated
            if not self.nodes[node].get("fsm") and node not in [
                "confirm_menu", "edit_menu", "building_type_menu",
                "interview_menu", "design_menu"
            ]
        }
...
def main():
    """Главная функция"""
    print("🌳 ПОСТРОИТЕЛЬ ГРАФА МЕНЮ VOXPERSONA v2")
    print("=" * 60)
    ...
    # Статистика
    print(f"\n📊 СТАТИСТИКА:")
    print(f"   Узлов (nodes): {len(builder.nodes)}")
    print(f"   Связей (edges): {len(builder.edges)}")
    ...
    # Специальные узлы
    dynamic = sum(1 for n in builder.nodes.values() if n.get("dynamic"))
    fsm = sum(1 for n in builder.nodes.values() if n.get("fsm"))
    print(f"\n   Динамические: {dynamic}")
    print(f"   FSM: {fsm}")

    # Условные связи
    conditional = sum(1 for e in builder.edges if e.get("condition"))
    print(f"   Условные связи (super_admin): {conditional}")
    ...
    print(f"✅ Сохранено: {output_path.stat().st_size / 1024:.2f} KB")

    # Примеры
    print(f"\n📋 ПРИМЕРЫ УЗЛОВ:")
    examples = ["menu_main", "chat_actions", "access_user_details", "new_chat"]
    ...
    print("✅ ГОТОВО!")
    print(f"\n📌 ИТОГ:")
    print(f"   ✅ {len(builder.nodes)} узлов")
    print(f"   ✅ {len(builder.edges)} связей")
    print(f"   ✅ {conditional} условных связей (super_admin)")
